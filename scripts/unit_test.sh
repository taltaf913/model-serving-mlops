...
./training/tests/test_run_experiment.sh || FAILURE=true  # || operator means "or", so if pytest fails (i.e., returns a non-zero status code), then the FAILURE variable is set to true.
